        ...
        <section class="topRanking">
            <div class="topRankingArea">
                ...
                <div class="rankingArea">
                    ...
                    <table class="topWordRanking">
                        @php
                        $scores = $scoresByLanguage[$language->id];
                        $dummyDataCount = 3 - count($scores); // 3位までの表示に足りないデータの数
                        @endphp
                        @for ($i = 0; $i < 3; $i++) @if ($i===0 && isset($scores[$i])) <tr class="firstRank">
                            <td class="rank1">{{ $i + 1 }}位</td>
                            <td class="name1">{{ $scores[$i]->username }}</td>
                            <td class="score1">{{ $scores[$i]->score }}点</td>
                            </tr>
                            ...
                            <tr>
                                <td class="rank1">{{ $i + 1 }}位</td>
                                <td class="name1">挑戦してね！</td>
                                <td class="score1">00点</td>
                            </tr>
                            @endif
                            @endfor
                    </table>
            ...
            <section class="topRanking2">
                ...
                <div class="topRankingArea2">
                    <div class="selectWrap2">
                        <a class="{{ $selectedLevel == 1 ? 'selected' : '' }}" href="{{ route('top') }}?level_id=1">初級コース</a>
                        <a class="{{ $selectedLevel == 2 ? 'selected' : '' }}" href="{{ route('top') }}?level_id=2">中級コース</a>
                    </div>
            ...
            <section class="infoNews">
                ...
                <ul class="infoList">
                    ...
                    <li><a href="{{ route('knowhow') }}?id={{ $item2->id }}">
                            <img alt="{{ $item2->title }}" class="info-pho" src="{{ asset('storage/images') }}/{{ $item2->thumbnail }}">
